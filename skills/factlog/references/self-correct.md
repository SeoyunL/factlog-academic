...
- 타입 리터럴(날짜·금액·순위·수치)을 값으로 일치시키는 query는 저장된 **compound term**
  형식을 써야 매칭됩니다 — `date(2030)`·`date(2030,1)`, `ordinal(3)`, `number(2.5)`,
  그리고 금액은 단위를 따옴표로 둔 `amount(100,"억")`(query 문자열 안에서는 `\"`로 escape:
  `"amount(100,\"억\")"`). 날짜는 저장된 정밀도를 그대로 써야 하며, 연도만 담긴
  `date(2030)`을 `date(2030,1)`로 늘려 적으면 매칭되지 않습니다. 프로즈(`"100억"`,
  `"2030.1"`)와 래퍼 없는 `"2030"`은 매칭되지 않으니, 값 추출이면
