...
`date(2030)`·`date(2030,1)`(날짜는 연·월·일 정밀도를 모두 받습니다),
`amount(100,"억")`(금액 단위는 항상 따옴표), `ordinal(3)`, `number(2.5)`.
object 인자는 저장된 문자열과 일치해야 하므로:
...
- **특정 값과 일치**하는지 묻는 질문("투자액이 100억이야?")은 object에 **compound term**을
  ...
  `date(2030)`·`date(2030,1)`·`ordinal(3)`·`number(2.5)` 형식 그대로 씁니다. 날짜는
  **저장된 정밀도 그대로** 적어야 합니다 — 연도만 아는 사실은 `date(2030)`으로 저장되고,
  같은 스칼라(`20300101`)로 투영되더라도 `date(2030,1)`과는 다른 문자열이라 서로
  매칭되지 않습니다. 프로즈(`"100억"`, `"2030.1"`, `"3등"`)와 래퍼 없는 `"2030"`은
  저장값과 일치하지 않아 검증되지 않습니다.
