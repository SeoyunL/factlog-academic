...
| `.html`, `.htm`, `.rtf` | **Auto-converted** | Text-based containers: the bytes are text but the content is markup, so they are converted rather than read raw. `.rtf` needs `textutil` (**macOS only**). |
