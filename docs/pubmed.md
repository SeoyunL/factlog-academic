...
그래서 인용부의 **탭과 모든 줄바꿈 문자**는 각각 **공백 한 칸으로 치환**된 뒤 출력됩니다
(`--porcelain` 행이 쓰는 것과 같은 규칙). 지우는 게 아니라 바꾸는 것이므로 감사에 필요한
원문은 그대로 남고, 블록은 두 줄로 유지됩니다.

여기서 "줄바꿈"은 LF·CR 만이 아니라 파이썬 `str.splitlines()` 가 줄바꿈으로 인정하는 집합
전체입니다. **U+0085(NEL)·U+2028·U+2029 는 C0 제어문자가 아니면서 XML 1.0 에서 합법이고
줄을 쪼갭니다** — "C0 만 막으면 된다"는 기준으로 짜면 정확히 이 셋이 새고, 그 상태로도 위조가
성립합니다.

줄을 쪼개지 않는 문자는 손대지 않습니다. `⚠` 도, U+007F(DEL) 같은 제어문자도 그대로 둡니다.
열이나 줄을 늘리지 못하는 문자는 이 규칙이 지키는 계약을 깨지 않으며, 거기까지 손대기
시작하면 "보기 좋게 만드는" 일이 되어 경계가 사라집니다.
...
- `--yes` — 확인 프롬프트를 건너뜁니다. 터미널이 아닌 환경에서 **기록**하려면 필수이며,
  없으면 비대화형 실행은 거부하고 아무것도 쓰지 않습니다. **철회를 기록할 수는 있어도 해제할
  수는 없습니다** (#106) — 해제는 `--yes` 를 붙여도 거부되므로 터미널이 필요합니다.
