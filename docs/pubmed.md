...
> **선택·`--all` 은 이제 실제로 `sources/` 에 임포트합니다** — #166의 공유 임포트
> 경로(`PubMedSourceWriter`)를 재사용합니다. 임포트된 레코드는 여전히 **후보**이며
> `/factlog sync → review → accept` 게이트를 거쳐야 사실이 됩니다(P1/P2). 터미널에서는
> 무엇을 가져올지 물어보고, 물을 수 없으면(비-TTY·`--porcelain`) 아무것도 선택하지
> 않으므로 전체를 가져오려면 `--all` 이 필요합니다.
