...
| 원장이 없고, front matter에 `arxiv_version` 도 없다 | **현재 어떤 명령도 고치지 못합니다** (#105) |

마지막 줄이 중요합니다. `arxiv-import` 는 `already imported (arxiv_id match)` 로 건너뛰고,
`arxiv-backfill-provenance` 는 버전 없는 논문을 `refused` 합니다. 고칠 수 없다고 말하는 것이
정직한 답이며, 여기서 아무 명령이나 이름 붙이는 것은 같은 거짓말을 장소만 옮기는 일입니다.
...
- front matter에 `arxiv_version` 이 **없으면**: 백필이 그 논문을 `refused` 하므로 원장이
  생기지 않고, **이 논문의 철회는 현재 어떤 명령으로도 종결할 수 없습니다** (#105). 백필의
  `required=("version",)` 이 그렇게 정해져 있으며, 그 이유는 아래 `arxiv-backfill-provenance`
  절에 있습니다.
