...
지연**을 factlog가 스스로 지킵니다 — 강제되지 않는 예의라(지연 없는 요청 열두 개가 모두
200으로 답했습니다) 아무도 대신 지켜 주지 않습니다.
...
- `--show-query` 는 **요청을 보내지 않고** 실제로 전송될 `search_query` 를 출력하고
  끝납니다. 출력되는 문자열은 클라이언트가 쓰는 것과 같은 composer가 만들기 때문에 실제
  전송값에서 표류할 수 없습니다.
...
**감싸기를 끄는 법.** 쿼리가 그 자체로 아무것도 표현하지 않을 때만 감쌉니다.

- **필드 프리픽스** (`ti:`, `au:`, `abs:`, `co:`, `jr:`, `cat:`, `rn:`, `id:`, `all:`) —
  이미 arXiv의 언어로 말하고 있으므로 그대로 둡니다.
...
- **이미 들어 있는 큰따옴표** — 의도적으로 인용한 것이므로 그대로 둡니다. 다만 따옴표
  개수가 홀수면 거부합니다. arXiv는 짝 없는 따옴표를 무시하고 느슨하게 매칭하므로, 이
  함수가 막으려는 바로 그 과다 매칭이 예고 없이 통과해 버립니다.

한 단어를 감싸는 것은 무의미한 연산입니다(`transformer` 와 `all:"transformer"` 둘 다
172,792편), 그래서 규칙을 균일하게 유지해도 비용이 없습니다. 감싸기가 일어나면 factlog는
무엇을 감쌌는지 stderr에 알립니다 — 운영자가 타이핑한 것을 조용히 고쳐 쓰는 일은 조용히
잘못 검색하는 것과 같은 배신입니다.
...
원장에 arXiv 레코드는 있는데 `version` 이 없는 경우가 있습니다(손으로 편집했거나, 외부에서
만든 원장이거나, `arxiv_id` 만 메아리치는 OpenAlex 임포트). 이 논문은 **`no-version`** 이라는
자기 자신의 상태로 보고됩니다 — `unchanged` 도 `changed` 도 아닙니다 (#121).

`unchanged` 가 아닌 이유: 비교된 것이 없습니다. 이 논문은 이 명령이 존재하는 이유인 그
신호에서 조용히 빠져 있었고, `Version changed: 0` 을 읽은 운영자는 그 논문에 수리가
필요하다는 것도 `--auto-update` 가 수리해 준다는 것도 알 길이 없었습니다. `changed` 도
아닌 이유: "버전이 None 에서 7로 바뀌었다"에서 `None` 은 파이썬 값이지 원장이 기록한 적
있는 버전이 아닙니다.
...
실행할 때마다 arXiv를 **실시간으로** 조회합니다. 기록할 값은 상류에만 있기 때문입니다.
원장과 arXiv의 값이 이미 같으면(둘 다 철회 없음이거나 같은 agent) 프롬프트도 쓰기도 없이
0으로 끝납니다.

**`--yes` 는 철회를 *기록*할 수는 있어도 *해제*할 수는 없습니다** (#106). 아래
[결정론 경계](#결정론-경계) 3번을 보세요.

원장이 없는 논문(front matter만 있는, #82 이전 임포트)은 종결할 수 없습니다 — 결정을 적을
곳이 없습니다. 먼저 `arxiv-backfill-provenance` 로 원장을 만드세요.
...
5. **임포트된 항목은 여전히 후보**입니다. `sync → review → accept` 게이트를 거쳐야 사실이
   됩니다 (P1/P2).
...
`sources/<slug>.md` 하나. YAML front matter + 본문입니다. 키는 값이 있을 때만 나타납니다.
...
`request_delay` 를 권고치보다 낮춰도 arXiv는 밀어내지 않습니다. 받아들이되 조용히 받지는
않습니다 — 위험은 낮춘 사람이 알고 집니다.
