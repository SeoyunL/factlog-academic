...
> 둘 다 아무것도 하기 전에 자기 자신과 대상을 차례로 밝힙니다.** 먼저
> `factlog: <버전> (<factlog/__init__.py 경로>)` 로 지금 실행 중인 코드를
> 밝히고(#554), 이어서 `<도구>: target KB <루트> (from <출처>)` 를 출력합니다 —
> 대상 줄은 손대 쓰는 도구들이 찍는 것과 같은 줄입니다. 둘 다
