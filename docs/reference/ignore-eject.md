...
> `--orphans` 는 **할 일이 남지 않은 ref** — 디스크에 지울 파일이 없고, **완전한**
> `runs/*.json` 행(subject·relation·object·source 가 모두 채워진 행, merge 가 실제로
> 쓰는 행)도 없고, 인용 행이 전부 `superseded` — 를 자동 스캔에서 건너뜁니다. 그래서
> 같은 명령을 다시 돌리면 `no orphaned sources found` 로 끝나 "이제 깨끗하다" 를 확인할
> 수 있습니다. 위 (1) 은 run 행이 아직 남아 있으므로 여기에 해당하지 않습니다.
> 불완전한 run 행만 남은 ref 도 이 스캔에서는 빠집니다 — merge 가 그런 행을
> `skip incomplete row in ...` 으로 버리므로 되살아날 일이 없기 때문입니다. 그 행까지
> `runs/*.json` 에서 지우려면, 또는 툼스톤까지 없애려면 `--purge` 를
> 주거나(`--orphans --purge` 도 됩니다) ref 를 직접 이름 대십시오 — 명시적으로 지정한
> ref 는 언제나 매칭됩니다.
