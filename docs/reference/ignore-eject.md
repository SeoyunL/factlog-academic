...
단, **같은 이름의 원본이 `sources/` 에 이미 있으면 아무것도 매칭하지 않고 종료 코드
1 로 끝납니다.** `ingest` 는 `sources/` 밖 원본의 파일명만 헤더에 적으므로, 평면
변환본은 그 이름을 가진 파일 중 *어느* 것에서 나왔는지 말할 수 없습니다. KB 가 그
이름의 원본을 직접 들고 있다면 그쪽이 답이고, `eject` 는 확인 프롬프트 없이 파일을
지우므로 모호한 지정은 거부합니다.
