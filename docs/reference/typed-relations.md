...
- `date` — `2030.1` / `2030-01-15` → 정렬 가능한 yyyymmdd. **엔진 투영 지원**
  (정렬/임계값/범위). 빠진 부분은 `01` 로 채워집니다. 연도만 아는 값은 compound
  term `date(2030)` 으로 씁니다(→ `20300101`). 반면 **맨 `2030`** 은 파싱되지
  않습니다 — 구분자도 `date(…)` 래퍼도 없으면 일반 수치와 구별할 수 없기
  때문입니다.
...
됩니다: `date(2030)`, `date(2030,1)`, `date(2030,1,15)`, `number(2.5)`, `ordinal(3)`,
