...
**Unreadable payloads.** The hook reads its target from `tool_input.file_path` in the `PreToolUse` payload (a bare top-level `file_path` is still accepted, for callers that pipe a raw tool input). If no target can be read at all, the hook does not simply wave the call through: it scans the raw payload for `facts/accepted.dl` or `facts/query.dl`. Finding neither, it allows — the payload demonstrably names no engine input. Finding one, it treats the write as targeting an engine input of unknown identity, which rules out the bootstrap exception (that needs a *named* file to test on disk) and leaves a fresh report as the only way through — so `/factlog check` remains the way out. Note the limits of that fallback: it recognises an engine input only by the two filenames appearing literally in the payload text, and having no readable path it cannot tell which KB was meant, so it judges against the active KB's report. It is a backstop for payload shapes the gate does not understand, not a second complete implementation of the predicate.
