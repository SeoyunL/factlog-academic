...
safe"), so its own failure modes matter as much as the engine's.

``PYTHON`` was assigned unconditionally, so ``PYTHON=<interpreter> bash
tests/golden.sh`` — the convention the other 38 harnesses in ``tests/`` follow —
was silently discarded. On a machine without ``/tmp/factlog-venv`` the run fell
through to a bare ``python3`` that may not have pyrewire, and Step 2 died for a
reason that had nothing to do with the branch under test.

Each case runs the real harness as a subprocess against a throwaway copy of
``examples/sample-kb``, with ``PYTHON`` pointed at a shim that fails on purpose,
and asserts the shim was actually reached — so a harness that ignores ``PYTHON``
