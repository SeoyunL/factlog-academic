...
> **예전 KB 업그레이드?** 변환본 파일명 규칙(#213: 어간 → 전체 이름 + 확장자)과 하위
> 디렉터리 미러링이 바뀌어, 옛 평면 변환본이 커버리지 갭으로 보일 수 있습니다.
> `factlog ingest --scan --force` 로 옮기고 남은 옛 변환본은 직접 지웁니다. 자세히는
> [docs/eject.md](docs/eject.md#flat-layout-migration) 참고.
...
- 철회 플래그는 factlog가 주장하는 사실이 아니라 **OpenAlex의 의견**입니다 — OpenAlex가
  ...
  로 종결.
- front matter만 있는 저작(#84 이전)은 종결할 원장이 없어 acknowledge가 거부하고
  `factlog openalex-backfill-provenance` 를 가리킵니다 — 이 명령이 front matter로 원장을
  만듭니다(네트워크 없음, `sources/*.md` 미변경). arXiv와 달리 잃는 값이 없어 — 원장의
  모든 필드가 front matter 키를 가지므로 — 사람이 먼저 손으로 채울 값이 없습니다.
...
- `amount` — `100억` / `1,000원` → 정수 기본 단위. **엔진 투영 지원**. 단위 표가
  필요합니다(내장 기본, 또는 인라인 선언).
- `number` — `1,000` / `3.5` → 수치 크기, 정렬 가능한 int64 로 스케일. **엔진 투영
  지원** — 단, 비교 임계값은 같은 스케일 단위로 적어야 합니다.

추출기는 타입 지정 객체를 compact compound term(`date(2030,1)`, `amount(100,"억")`
…)으로 써도 됩니다. `number` 의 정확한 ×1000 스케일·`ROUND_HALF_UP` 규칙, `amount`
단위 표 문법, compound term 형태, 비교 술어 작성법은
**[docs/typed-relations.md](docs/typed-relations.md)** 참고.
...
변환본을 건드리지 않습니다. 출처에 귀속할 수 없는 변환본(옛 평면 변환본, `sources/`
밖에서 만든 것, 원본이 삭제된 것)은 추측 없이 그대로 두고 `--delete-original` 이
거부합니다 — 자세히는 [docs/eject.md](docs/eject.md) 참고. `pages/` 는 `eject` 로
재생성되지 않습니다 —
