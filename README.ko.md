...
접힘 충돌이 *분열*인지 *중복 레코드*인지는 추측이 아니라 정책이 정합니다.
`policy/attribute-relations.md` 에 선언된 관계에서는 값이 주체를 식별하므로(제목·DOI),
두 주체가 한 값을 공유하면 중복 레코드일 가능성이 큽니다. 그 밖의 관계에서는 값이 여러
주체에 공유되는 게 정상이므로, 거기서의 충돌은 실제 질의 누수입니다. `--strict` 는
누수에만 실패합니다.
